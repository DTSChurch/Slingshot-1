...
namespace Slingshot.PCO.Models
{
    ...
    public class PCORelationships
    {
        ...
        public PCOItemsResult event_periods { get; set; }
        public PCOItemsResult location { get; set; }
        public PCOItemsResult event_times { get; set; }
        [JsonProperty("event")]
        public PCOItemsResult my_event { get; set; }
        public PCOItemsResult checked_in_by { get; set; }
    }
    ...
    public class PCOEvent
    {
        ...
        public DateTime? created_at { get; set; }
        public bool? enable_services_integration { get; set; }
        public string frequency { get; set; }
        public string integration_key { get; set; }
        public DateTime? updated_at { get; set; }
        public List<PCOEventPeriod> event_periods { get; set; }
        public PCOEvent( PCOData data )
        {
            id = data.id;
            name = data.Item.name;
            archived_at = data.Item.archived_at;
            created_at = data.Item.created_at;
            enable_services_integration = data.Item.enable_services_integration;
            frequency = data.Item.frequency;
            integration_key = data.Item.integration_key;
            updated_at = data.Item.updated_at;
            event_periods = new List<PCOEventPeriod>();
        }
        public void UpdateEventPeriods( PCOData data, List<PCOData> included )
        {
            if ( data.relationships != null )
            {
                if ( data.relationships.event_periods != null )
                {
                    foreach ( var relationship in data.relationships.event_periods.data )
                    {
                        var item = included
                            .Where( i =>
                                i.type == relationship.type &&
                        ...
                        if ( item != null )
                        {
                            event_periods.Add( new PCOEventPeriod( item ) );
                        }
                    }
                }
            }
    ...
    public class PCOLocation
    {
        public int id { get; set; }
        public string name { get; set; }
        public DateTime? created_at { get; set; }
        public DateTime? updated_at { get; set; }
        public PCOLocation( PCOData data )
        {
            id = data.id;
            name = data.Item.name;
            created_at = data.Item.created_at;
    ...
    public class PCOEventTime
    {
        public int id { get; set; }
        public DateTime? created_at { get; set; }
        public int? guest_count { get; set; }
        public DateTime? hides_at { get; set; }
        public int? regular_count { get; set; }
        public DateTime? shows_at { get; set; }
        public DateTime? starts_at { get; set; }
        public int? total_count { get; set; }
        public DateTime? updated_at { get; set; }
        public int? volunteer_count { get; set; }
        public PCOEventTime( PCOData data )
        {
            id = data.id;
            created_at = data.Item.created_at;
            guest_count = data.Item.guest_count;
            hides_at = data.Item.hides_at;
            regular_count = data.Item.regular_count;
            shows_at = data.Item.shows_at;
            starts_at = data.Item.starts_at;
            total_count = data.Item.total_count;
            updated_at = data.Item.updated_at;
            volunteer_count = data.Item.volunteer_count;
        }
    }

    public class PCOEventPeriod
    {
        public int id { get; set; }
        public DateTime? created_at { get; set; }
        ...
        public DateTime? updated_at { get; set; }
        public int? volunteer_count { get; set; }
        public PCOEventPeriod ( PCOData data )
        {
            id = data.id;
            created_at = data.Item.created_at;
            ends_at = data.Item.ends_at;
            guest_count = data.Item.guest_count;
            note = data.Item.note;
            regular_count = data.Item.regular_count;
            starts_at = data.Item.starts_at;
    ...
    public class PCOCheckIn
    {
        ...
        public string kind { get; set; }
        public int? person_id { get; set; }
        public PCOLocation location { get; set; }
        public List<PCOEventTime> event_times { get; set; }
        public PCOEventPeriod event_period { get; set; }
        public PCOEvent my_event { get; set; }
        public int? checked_in_by_id { get; set; }
        public PCOCheckIn( PCOData data )
        {
            id = data.id;
            checked_out_at = data.Item.checked_out_at;
            created_at = data.Item.created_at;
            kind = data.Item.kind;
            if ( data.relationships != null )
            {
                ...
                if ( data.relationships.checked_in_by != null )
                {
                    foreach ( var relationship in data.relationships.checked_in_by.data )
                    {
                        if ( relationship != null )
                        {
                            checked_in_by_id = relationship.id;
                        }
                    }
                }
            }
            event_times = new List<PCOEventTime>();

        }
        public void Updatedata ( PCOData data, List<PCOData>included )
        {
            
            if ( data.relationships != null )
            {
                
                if ( data.relationships.location != null )
                {
                    foreach ( var relationship in data.relationships.location.data )
                    {
                        if ( relationship != null )
                        {
                            ...
                            if ( item != null )
                            {
                                location = new PCOLocation( item );
                            }
                        }
                    }
                }
                
                if ( data.relationships.event_times != null )
                {
                    foreach ( var relationship in data.relationships.event_times.data )
                    {
                        var item = included
                            .Where( i =>
                                i.type == relationship.type &&
                        ...
                        if ( item != null )
                        {
                            event_times.Add( new PCOEventTime( item ) );
                        }
                    }
                }
                if ( data.relationships.event_periods != null )
                {
                    foreach ( var relationship in data.relationships.event_periods.data )
                    {
                        if( relationship != null )
                        {
                            ...
                            if ( item != null )
                            {
                                event_period = new PCOEventPeriod( item );
                            }
                        }
                    }
                }
                if ( data.relationships.my_event != null )
                {
                    foreach ( var relationship in data.relationships.my_event.data )
                    {
                        if ( relationship != null )
                        {
                            var item = included
